...
/**
 * <B>@gm, we changed the reference of map object to weak reference, so we DO
 * NOT promise to maintain the cache.</B><br> Static library version of
 * {@link android.util.LruCache}. Used to write apps that run on API levels
 * prior to 12. When running on API level 12 or above, this implementation
 * is still used; it does not try to switch to the framework's
 * implementation. See the framework SDK documentation for a class
 * overview.
 */
public class LruCache<K, V> {
    ...
    private LinkedHashMap<K, V> getMap() {
        if (map == null) {
            this.map = new LinkedHashMap<K, V>(0, 0.75f, true);
            size = 0;
        }
    ...
    public final V get(K key) {
        ...
        synchronized (this) {
            createCount++;
            mapValue = getMap().put(key, createdValue);

            if (mapValue != null) {
                // There was a conflict so undo that last put
                getMap().put(key, mapValue);
            } else {
    ...
    public void trimToSize(long maxSize) {
        while (true) {
            ...
            synchronized (this) {
                ...
                if (size <= maxSize || getMap().isEmpty()) {
                    break;
                }

                Map.Entry<K, V> toEvict = getMap().entrySet().iterator().next();
                key = toEvict.getKey();
    ...
    protected void entryRemoved(boolean evicted,
                                ...
                                V oldValue,
                                V newValue) {
        //        if (evicted && oldValue != null && oldValue instanceof Bitmap && !((Bitmap) oldValue).isRecycled()) {
