 ...
 */
public class EnDeTypeBigEndian {
    ...
    public static void copyIntToBytes(int num, byte[] bytes, int pos) {
        for (int i = 3; i >= 0; --i) {
            bytes[pos + 3 - i] = (byte) (num >>> (i * 8));
        }
    }

    public static int bytesToInt(final byte[] bytes, int pos) {
        int num = 0;
        for (int i = 3; i >= 0; --i) {
            num |= (bytes[pos + i] & 0xFF) << ((3 - i) * 8);
        }
        return num;
    }
}
